...
def binary_feature_trans(features, counts, dataset):
    Bond = [0 for _ in range(len(features))]
    for i in range(len(features)):
        bond = 0
        try:
            a = [int(j) for j in features[i]]
            features[i] = a
            label0 = 0
            label1 = 0
            bond = np.median(np.array(features[i]))
            Bond[i] = bond
            for j in range(len(features[i])):
                if features[i][j] <= np.median(np.array(features[i])):
                    label0 += counts[i][j]
                else:
                    label1 += counts[i][j]
            new_counts = [label0, label1]
            # print(i, new_counts, '\n')
            features[i] = ['yes', 'no']
            counts[i] = new_counts
            # print(i, '\n')
            # print(i, np.median(np.array(a)), '\n')
        except ValueError:
            pass
        if bond != 0:
            for data in dataset:
                if int(data[i]) <= bond:
                    ...
                    data[i] = 'no'
    return features, counts, dataset
...
def information_gain(new_features, counts, dataset, labels, E, features):  # E (Entropy) is the total Entropy for dataset S
    ...
    for i in range(len(new_features[:-1])):
        ...
        # print(i, len(f_data), new_features[i])
        ...
        for sub_data in f_data:
            ...
            if len(sub_data) != 0:
                a = get_num_feature(sub_data, labels, -2)
                # print(i, 'En data: ', a)
                ...
                # print(i, 'En[sub]: ', En[f_data.index(sub_data)])
...
def Add_org_weights(dataset):
    for data in dataset:
        data.append(1/len(dataset))
    return dataset
...
def training(train_dataset, tree_depth):
    ...
    for i in range(tree_depth):
        ...
        for j in range(len(train_dataset)):
            ...
            # print(i, len(Gain), max(Gain), Gain.index(max(Gain)), Gain)
    ...
    #     print(d, len(train_dataset[d]), train_dataset[d])
...
def get_pred_label(data, tree, i_d):
    for key in tree.keys():
        if data[i_d] == list(key)[i_d]:
            pred_label = list(tree[key])[0]
    return pred_label

def weights_error(y, y_pred, weights):
    count = 0
...
def find_pred_and_label(dataset, tree):
    true_label = []
    pred_label = []
    i_d = list(tree.values())[0][1][0]
    for data in dataset:
        true_label.append(data[-2])
        pred_label.append(get_pred_label(data, tree, i_d))
    return true_label, pred_label

def weight_update(y, y_pred, w, alpha):
    new_weights = []
    for i in range(len(y)):
        if y[i] == y_pred[i]:
            a = 1
            ...
            a = -1
        new_weights.append(w[i]*math.e**(-alpha*a))
    new_weights = [weight/sum(new_weights) for weight in new_weights]
    return new_weights

def data_weights_update(data, weights):
    for i in range(len(data)):
        data[i][:-1] = weights[i]
    return data

def Ada_boost(iter, dataset):
    vote = []
    weights = [w[:-1] for w in dataset]
    for i in range(iter):
        tree, dataset = training(dataset, 1)
        true_label, pred_label = find_pred_and_label(dataset, tree)
        error = weights_error(true_label, pred_label, weights)
        print(i, error)
        alpha = 0.5*math.log((1-error)/error)
        vote.append(alpha)
        weights = weight_update(true_label, pred_label, weights, alpha)
        dataset = data_weights_update(dataset, weights)
    return vote, weights

def Attend_tree(dataset, features):
    pass
